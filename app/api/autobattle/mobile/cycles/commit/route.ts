...
import { commitAutoBattleCycle } from "@/app/autobattle-db";
...
export async function POST(request: Request) {
  try {
    ...
    return noStoreJson({ ok: true, reservation: await commitAutoBattleCycle(session.user_id, reservationId) });
  } catch (error) {
    console.error("AutoBattle cycle commit failed", error);
