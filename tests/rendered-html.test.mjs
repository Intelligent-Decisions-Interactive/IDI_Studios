...
test("keeps the IDI Studios voice and Conquest focus in the homepage source", async () => {
  ...
  assert.match(page, /<strong>Command<\/strong>/);
  assert.match(styles, /\.conquest-signal > span \{[^}]*container-type: inline-size;/);
  assert.match(styles, /\.conquest-signal strong \{[^}]*font-size: clamp\(1\.55rem, 16\.5cqi, 4rem\);[^}]*white-space: nowrap;/);
  assert.match(styles, /\.hero h1 \{ font-size: clamp\(3\.2rem, 16cqi, 4\.2rem\);/);
...
test("stores verified beta applications in Supabase and sends notifications", async () => {
  ...
  assert.match(form, /0x4AAAAAAEFhAAW5N5kUh-aO/);
...
test("protects and operates the beta administration console", async () => {
  const [
    ...
    await Promise.all([
      ...
      readFile(
        new URL("../app/beta/admin/api/requests/route.ts", import.meta.url),
        "utf8",
      ),
      readFile(
