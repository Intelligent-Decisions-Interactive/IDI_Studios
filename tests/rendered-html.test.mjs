...
test("keeps the IDI Studios voice and Conquest focus in the homepage source", async () => {
  ...
  assert.match(styles, /\.conquest-signal span \{[^}]*font-size: clamp\(2\.1rem, 4vw, 4rem\);[^}]*white-space: nowrap;/);
  assert.match(styles, /\.hero h1 \{ font-size: clamp\(3\.4rem, 17vw, 4\.5rem\);/);
  assert.match(styles, /\.conquest-signal span \{[^}]*font-size: clamp\(1\.55rem, 7\.2vw, 2rem\); letter-spacing: 0;/);
